...
# Some fixtures under android/vectors/ are 3.0 artifacts this released-wire
# generator (v2.0.0) does not and cannot produce — SPT did not exist in v2.0.0 —
# so they are excluded from the v2.0.0 comparison. Each carries its OWN
# cross-language conformance proof; none is part of the v2.0.0 snapshot:
#   - deployment-evaluator-v3.json: generated by scripts/gen-evaluator-corpus.ts
#     from src/claims/shannon-deployment.ts; proven by DeploymentCorpusTest and
#     tests/deployment-evaluator-corpus.test.ts.
#   - spt-interop.json: the Sealed Pad Transfer interop corpus, generated by
#     scripts/gen-spt-interop.ts from the released SPT; proven by SptInteropTest
#     and tests/spt-interop-corpus.test.ts (the TS side re-seals and compares to
#     the committed packageHex, so a stale corpus cannot survive).
...
if (( check )); then
  if diff -ru "${exclude[@]}" "$android/vectors" "$out" >/dev/null; then
    ...
    diff -ru "${exclude[@]}" "$android/vectors" "$out" >&2 || true
    exit 1
  fi
else
