...
describe("letter mode", () => {
  it("encrypts with add-mod-26 against known pad symbols", () => {
    ...
    const result = encryptLetters("ABC", pad, "A");
  ...
  it("round-trips through a serialized receiver copy of the pad", () => {
    ...
    const encrypted = encryptLetters("Attack at dawn, bring the LANTERNS!", sender, "A");
    expect(encrypted.ok).toBe(true);
    if (!encrypted.ok) return;
    const decrypted = decryptLetters(encrypted.envelope, receiver, "B");
  ...
  it("burns disjoint offsets across consecutive messages", () => {
    ...
    const first = encryptLetters("MEETMEATNOON", sender, "A");
    const second = encryptLetters("BRINGTHEBOOK", sender, "A");
    ...
    const firstPlain = decryptLetters(first.envelope, receiver, "B");
    const secondPlain = decryptLetters(second.envelope, receiver, "B");
  ...
  it("refuses when the pad is short — and burns nothing", () => {
    ...
    const result = encryptLetters("THISMESSAGEISTOOLONG", pad, "A");
  ...
  it("decrypt refuses on a device whose pad copy is exhausted", () => {
    ...
    pad.consume(4);
    const result = decryptLetters({ label: pad.label, startOffset: 4, consumed: 5, payload: "XYZZY" }, pad, "B");
...
describe("byte mode", () => {
  it("XOR round-trips arbitrary bytes", () => {
    ...
    expect(encrypted.ok).toBe(true);
    if (!encrypted.ok) return;
    const decrypted = decryptBytes(encrypted.envelope, receiver, "B");
    expect(decrypted.ok).toBe(true);
    if (!decrypted.ok) return;
  ...
  it("refuses when the pad is short", () => {
    ...
    const result = encryptBytes(new Uint8Array(10), pad, "A");
    expect(!result.ok && result.reason === "pad-exhausted").toBe(true);
  ...
  it("refuses a letters pad in byte mode", () => {
    ...
    const result = encryptBytes(new Uint8Array([1, 2, 3]), pad, "A");
    expect(!result.ok && result.reason === "mode-mismatch").toBe(true);
  });
});
...
describe("no keystream reuse at the cipher level", () => {
  it("encrypting the same plaintext twice uses different pad symbols", () => {
    ...
    const first = encryptLetters("SAMEMESSAGE", pad, "A");
    const second = encryptLetters("SAMEMESSAGE", pad, "A");
    expect(first.ok && second.ok).toBe(true);
    if (!first.ok || !second.ok) return;
