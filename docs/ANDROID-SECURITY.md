# TruePad 3 Android Edition — security & durability claims
...
**Two qualifications travel with that ceremony everywhere it is cited, and must
not be dropped here either:** the word comparisons were read from the two screens
by AUTOMATION, not spoken between two people, and the message round trips used
host/test carriers — only the SPT gates were optical.
