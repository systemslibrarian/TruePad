...
| 6 | Three-counter rollback witness; a regressed store refuses `witness-regressed` before consuming anything | PROTOCOL (the record + refusal) + PLATFORM-OP (where it lives) | BROWSER-OP: `browser-none` or `browser-independent-store` (a second, separately-cleared OPFS store), §4 | forthcoming | forthcoming |
